...
/////////////////////////////////////////////////////////////////
// utility functions, by @tomas.

char* get_string_from_token(char * json, jsmntok_t* token) {
...
int get_nested_token_count(jsmntok_t* token) {
	int end = token->end;
	int count = 0;
	token++;
...
jsmntok_t* get_hash_token(char * json, jsmntok_t * tokens, const char* wanted) {
	if (tokens == 0 || wanted == 0)
	  return 0;

  int offset = 1;
  if (tokens[0].type == JSMN_PRIMITIVE) {
    offset = 0; // not an array or object
  }
	...
	for (int i = 0; i < size; i++) {
		...
		current += 2 + get_nested_token_count(current + 1);
	}
...
jsmntok_t* get_array_token(char * json, jsmntok_t * tokens, int index) {
