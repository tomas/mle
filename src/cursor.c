...
int cursor_drop_anchor(cursor_t* cursor, int use_srules) {
  if (cursor->is_anchored) return EON_OK;
...
int cursor_select_by(cursor_t* cursor, const char* strat) {
  ...
  if (strcmp(strat, "bracket") == 0) {
    ...
    EON_RETURN_ERR(cursor->bview->editor, "Unrecognized cursor_select_by strat '%s'", strat);
  }
...
int cursor_select_by_bracket(cursor_t* cursor) {
  ...
  if (mark_move_bracket_top(cursor->mark, EON_BRACKET_PAIR_MAX_SEARCH) != MLBUF_OK) {
    mark_destroy(orig);
    return EON_ERR;
  }
  ...
  if (mark_move_bracket_pair(cursor->anchor, EON_BRACKET_PAIR_MAX_SEARCH) != MLBUF_OK) {
    cursor_toggle_anchor(cursor, 0);
...
int cursor_select_by_word_back(cursor_t* cursor) {
  if (mark_is_at_word_bound(cursor->mark, -1)) return EON_ERR;

  cursor_toggle_anchor(cursor, 0);
  mark_move_prev_re(cursor->mark, EON_RE_WORD_BACK, sizeof(EON_RE_WORD_BACK) - 1);
  return EON_OK;
}
...
int cursor_select_by_word_forward(cursor_t* cursor) {
  ...
  cursor_toggle_anchor(cursor, 0);
  mark_move_next_re(cursor->mark, EON_RE_WORD_FORWARD, sizeof(EON_RE_WORD_FORWARD) - 1);
  return EON_OK;
}
...
int cursor_select_by_word(cursor_t* cursor) {
  ...
  if (mark_is_at_eol(cursor->mark)) return EON_ERR;

  mark_get_char_after(cursor->mark, &after);
  if (!isalnum((char)after) && (char)after != '_') return EON_ERR;
...
int cursor_replace(cursor_t* cursor, int interactive, char* opt_regex, char* opt_replacement) {
  ...
  do {
    ...
    while (1) {
      ...
      if (mark_find_next_re(search_mark, regex, strlen(regex), &bline, &col, &char_count) == MLBUF_OK
        ...
        if (!yn) {
        ...
        } else if (0 == strcmp(yn, EON_PROMPT_YES) || 0 == strcmp(yn, EON_PROMPT_ALL)) {
          ...
          if (0 == strcmp(yn, EON_PROMPT_ALL)) all = 1;
  ...
  if (interactive) {
    EON_SET_INFO(cursor->bview->editor, "replace: Replaced %d instance(s)", num_replacements);
    bview_rectify_viewport(cursor->bview);
    bview_draw(cursor->bview);
  }
